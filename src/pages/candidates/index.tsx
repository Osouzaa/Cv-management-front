  ...
  const Candidate = () => {
    ...
    const [showAlternativeContent, setShowAlternativeContent] = useState(false); // Novo estado
    ...
    return (
      <>
        <C.Container>
          {/* Botão para alternar entre as renderizações */}
          <button onClick={toggleContent}>
            {showAlternativeContent ? "Mostrar Lista" : "Mostrar Outra Coisa"}
          </button>

          <C.ContainerGrid>
            {/* Renderização baseada no estado showAlternativeContent */}
            {showAlternativeContent ? (
              <C.ContainerGrid>
              <ListCandidates />
              <button onClick={toggleContent}> VOLTAR</button></C.ContainerGrid>
            ) : currentData ? (
